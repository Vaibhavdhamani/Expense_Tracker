...
function ExpenseForm({ categories, userId, apiBase, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (formData.category_id) fetchStandardDescriptions(formData.category_id);
    else setStandardDescriptions([]);
  ...
  const fetchStandardDescriptions = async (categoryId) => {
    try {
      ...
      if (data.success) setStandardDescriptions(data.data);
    } catch (error) { console.error('Failed to fetch descriptions'); }
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.description)                newErrors.description = 'Please select or enter a description';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${apiBase}/expenses`, {
        ...
        body: JSON.stringify({ ...formData, user_id: userId, amount: parseFloat(formData.amount), date: new Date(formData.date).toISOString() })
      });
      const data = await response.json();
      if (data.success) onSuccess();
      else setErrors({ submit: data.error || 'Failed to add expense' });
    } catch (error) {
      ...
      <div className="modal-content expense-form-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div className="modal-title-section">
            <div className="modal-icon"><DollarSign size={24} /></div>
            <div>
        ...
        <form onSubmit={handleSubmit} className="expense-form">
          <div className="form-group">
            <label className="form-label"><Tag size={16} /><span>Category</span></label>
          ...
          {formData.category_id && (
            <div className="form-group animate-in">
              <label className="form-label"><FileText size={16} /><span>Description</span></label>
          ...
          {formData.description === '__other__' && (
            <div className="form-group animate-in">
              <label className="form-label"><FileText size={16} /><span>Custom Description</span></label>
              <input type="text" name="description" placeholder="Enter custom description..." value="" onChange={handleChange} className="form-input" autoFocus />
            </div>
          )}

          <div className="form-row">
            <div className="form-group">
              <label className="form-label"><DollarSign size={16} /><span>Amount</span></label>
              <div className="input-with-icon">
                <span className="input-icon">₹</span>   {/* ← ₹ */}
                <input type="number" name="amount" value={formData.amount} onChange={handleChange}
                  placeholder="0.00" step="0.01" min="0" className="form-input with-icon" />
              </div>
              {errors.amount && <span className="error-message">{errors.amount}</span>}
            </div>
            <div className="form-group">
              <label className="form-label"><Calendar size={16} /><span>Date &amp; Time</span></label>
              <input type="datetime-local" name="date" value={formData.date} onChange={handleChange} className="form-input" />
              {errors.date && <span className="error-message">{errors.date}</span>}
            </div>
          </div>

          <div className="form-group">
            <label className="form-label"><FileText size={16} /><span>Notes (Optional)</span></label>
            <textarea name="notes" value={formData.notes} onChange={handleChange}
              placeholder="Add any additional details..." rows="3" className="form-textarea" />
          </div>
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="btn-submit" disabled={loading}>
              {loading ? (
                <><div className="btn-spinner"></div><span>Adding...</span></>
              ) : (
                <><DollarSign size={18} /><span>Add Expense</span></>
              )}
            </button>
          </div>
        </form>
